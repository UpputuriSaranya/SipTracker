...
export default function InvestorHoldingsPage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchHoldings = async () => {
      try {
        console.log(`Fetching from: ${API_BASE}/api/investors/${investor_id}/holdings`);
        const response = await fetch(`${API_BASE}/api/investors/${investor_id}/holdings`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!response.ok) {
          throw new Error(`Unable to fetch holdings. Status: ${response.status}`);
        }
  ...
  return (
    <div className="space-y-6">
      <Header 
        ...
        subtitle="Review all assets and investments belonging to this investor." 
      />
      {loading ? (
                ...
                <tr key={holding?.id || holding?.asset_name || idx} className="hover:bg-slate-50">
                  ...
                  <td 
                    className={`px-6 py-4 text-sm font-medium ${
                      Number(holding?.profit_loss) >= 0 ? "text-emerald-600" : "text-rose-600"
                    }`}
                  >
