...
export default function InvestorNetworthPage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchNetworth = async () => {
      try {
        console.log(`Fetching from: ${API_BASE}/api/investors/${investor_id}/networth`);
        const response = await fetch(`${API_BASE}/api/investors/${investor_id}/networth`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!response.ok) {
          throw new Error(`Unable to fetch net worth. Status: ${response.status}`);
        }
  ...
  return (
    <div className="space-y-6">
      <Header 
        ...
        subtitle="View a snapshot of portfolio value and asset allocations." 
      />
      {loading ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-8 text-slate-700 shadow-sm">
          Loading net worth summary...
        </div>
      ) : error ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p 
                className={`mt-4 text-3xl font-semibold ${
                  Number(totalProfitLoss) >= 0 ? "text-emerald-600" : "text-rose-600"
                }`}
              >
                  ...
                  <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 p-4">
                    <div>
                      <p className="text-sm font-medium text-slate-900">
                        {item?.asset_type || item?.name || `Asset ${idx + 1}`}
                      </p>
